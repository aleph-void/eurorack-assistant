...
const PATCH_MODULES = [
  ...
  // An instance whose module record is gone: still in the patch, but not a
  // module a clip could be attached to.
  {
    id: 13,
    module_id: null,
    manufacturer: 'Bygone',
...
describe('ScopePanel', () => {
  ...
  it('will not record with every channel unticked', async () => {
    api.get.mockImplementation(async (path) =>
      path.startsWith('/api/scope') ? { patch_id: 7, channels: CHANNELS, devices: [] } : []
    );
  ...
  it('records whatever the scope has when no channels are known, without inventing a duration', async () => {
    api.get.mockImplementation(async (path) =>
      path.startsWith('/api/scope') ? { patch_id: 7, channels: [], devices: [] } : []
    );
    api.post.mockResolvedValue({
      id: 11,
      // A module this patch has no instance of, so the status cannot name it.
      module_id: 999,
      captured_at: '2026-08-12T18:00:00Z',
      duration_seconds: 10,
      channels: [],
    ...
    await wrapper.find('[data-test="clip-duration"]').setValue('');
    await wrapper.find('[data-test="scope-record"]').trigger('click');
    await flushPromises();
    // With nothing to tick, the server picks the panes; an emptied duration
    // field is left to the server's default the same way.
    expect(api.post).toHaveBeenCalledWith('/api/scope/patches/7/clips', {
      connection_id: 3,
      channels: undefined,
    ...
    expect(wrapper.find('[data-test="scope-status"]').text()).toContain(
      'the module’s Videos page'
    );
  });

  it('surfaces a recording the device refused', async () => {
    api.get.mockImplementation(async (path) =>
      path.startsWith('/api/scope') ? { patch_id: 7, channels: CHANNELS, devices: [] } : []
    );
    ...
    await wrapper.find('[data-test="scope-record"]').trigger('click');
    await flushPromises();
    expect(wrapper.find('[data-test="scope-error"]').text()).toContain('already in progress');
  });

  it('keeps a clip when the delete is declined, and shows why one failed', async () => {

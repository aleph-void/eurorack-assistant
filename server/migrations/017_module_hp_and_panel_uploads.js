...
export const description = 'module HP, and panels a user supplied';
...
export async function down({ comment, dropColumn }) {
  await comment('COLUMN module_panels.source', null);
  // The widths this migration copied over from module_panels go with the
  // column that holds them; module_panels.hp, where they came from, stays.
  await dropColumn('modules', 'hp');
}

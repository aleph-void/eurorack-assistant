...
export const description = 'patch topology: groups, external gear, links, cable notes';
...
export async function down({ dropColumn, dropTable }) {
  await dropColumn('patch_cables', 'note', 'optional', 'stacked', 'alt_group');
  await dropTable('patch_module_link_jacks', 'patch_module_links', 'patch_module_ports');
  await dropColumn('patch_modules', 'label', 'group_id', 'external');
  await dropTable('patch_groups');
}

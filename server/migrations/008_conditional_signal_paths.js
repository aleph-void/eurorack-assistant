...
export const description = 'conditional routes and normalizations';

export async function up({ addColumn }) {
  const condition = 'INTEGER REFERENCES module_components(id) ON DELETE CASCADE';

  await addColumn('component_routes', 'condition_component_id', condition);
  await addColumn('component_routes', 'condition_value', 'TEXT');
  await addColumn('component_routes', 'alt_group', 'TEXT');

  await addColumn('component_normalizations', 'condition_component_id', condition);
  await addColumn('component_normalizations', 'condition_value', 'TEXT');
  await addColumn('component_normalizations', 'alt_group', 'TEXT');
}

export async function down({ dropColumn }) {
  await dropColumn('component_normalizations', 'condition_component_id', 'condition_value', 'alt_group');
  await dropColumn('component_routes', 'condition_component_id', 'condition_value', 'alt_group');
}

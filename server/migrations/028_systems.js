...
export const description = 'systems: racks patched together as one instrument';
...
export async function down({ dropColumn, dropIndex, dropTable }) {
  await dropIndex('racks_user_idx');
  await dropColumn('patch_modules', 'rack_id', 'rack_name');
  await dropColumn('patches', 'system_id', 'system_name');
  await dropIndex('racks_system_idx');
  await dropColumn('racks', 'system_id', 'system_x', 'system_y', 'system_position');
  await dropTable('systems');
}

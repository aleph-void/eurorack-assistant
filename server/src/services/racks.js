...
// Whether a component may be marked as a place sound leaves a rack or a
// system: it has to be one of the module's own jacks, because sound leaves
// through a cable. Answers { component } or { error } (a 400's message) —
// the rack and the system routes ask the same question.
export async function outputJack(db, moduleId, componentId) {
  const component = await db.models.ModuleComponent.findOne({
    where: { id: Number(componentId) || 0, module_id: moduleId },
  });
  if (!component) return { error: 'that component does not belong to this module' };
  if (!component.type.endsWith('_jack')) {
    return { error: 'an output has to be a jack — sound leaves through a cable' };
  }
  return { component };
}

// A rack joining a system brings the exits marked on it, so a case that was
// patched towards its own output module keeps that exit in the studio. Rows
// the system already holds are left as they are.
export async function carryRackOutputs(db, rack, systemId) {
  const { RackOutput, SystemOutput } = db.models;
  const marked = await RackOutput.findAll({
  ...
  const held = await SystemOutput.findAll({ where: { system_id: systemId } });
  const key = (o) => `${o.rack_id}:${o.module_id}:${o.component_id}`;
  const have = new Set(held.map(key));
  let position = held.reduce((max, o) => Math.max(max, o.position), 0);
  const rows = marked
    .filter((o) => !have.has(key(o)))
    .map((o) => ({
      system_id: systemId,
      rack_id: rack.id,
  ...
  if (rows.length > 0) await SystemOutput.bulkCreate(rows);
}

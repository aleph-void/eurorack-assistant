...
export function scopeRoutes(db, { hub = null, capturesDir = process.env.CAPTURES_DIR || '/data/captures' } = {}) {
  ...
  router.get('/patches/:id', async (req, res, next) => {
    try {
  ...
  router.post('/patches/:id/automap', async (req, res, next) => {
    try {
      const patch = await ownPatch(req.user.id, req.params.id);
      if (!patch) return res.status(404).json({ error: 'Patch not found' });
  ...
  router.put('/patches/:id/channels/:index', async (req, res, next) => {
    try {
      const patch = await ownPatch(req.user.id, req.params.id);
      if (!patch) return res.status(404).json({ error: 'Patch not found' });
  ...
  router.delete('/patches/:id/channels/:index', async (req, res, next) => {
    try {
      const patch = await ownPatch(req.user.id, req.params.id);
      if (!patch) return res.status(404).json({ error: 'Patch not found' });
  ...
  router.post('/patches/:id/captures', async (req, res, next) => {
    try {
      const patch = await ownPatch(req.user.id, req.params.id);
      if (!patch) return res.status(404).json({ error: 'Patch not found' });
      const device = pickDevice(req.user.id, req.body?.connection_id);
